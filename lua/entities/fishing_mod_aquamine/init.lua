...
timer.Create("AquaMine:FindTarget", 1, 0, function() 
	local number_of_mines = #ents.FindByClass("fishing_mod_aquamine")
	if number_of_mines == 0 then return end
	local mine = ents.FindByClass("fishing_mod_aquamine")[math.random(number_of_mines)]
	for key, entity in pairs(ents.FindInSphere(mine:GetPos(), 10000)) do
		if entity:GetClass() == "fishing_mod_angry_baby" and not entity.dead and entity:WaterLevel() >= 1 then
...
function ENT:Think()
	...
	if self.NextShock < CurTime() then
		...
		for _,ent in pairs(entsphere) do
			if ent:GetClass() == "fishing_mod_angry_baby" or ent:IsPlayer() then
